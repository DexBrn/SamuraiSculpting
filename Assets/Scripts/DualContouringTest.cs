...
public class DualContouringTest : MonoBehaviour
{
    ...
    // FIX 1: Density must CROSS the iso level at the surface.
    // Interior = +1, exterior = -1, surface = boundary voxels.
    void GenerateCuboid()
    {
            ...
            for (int y = 0; y < MDims.y; y++)
                for (int z = 0; z < MDims.z; z++)
                {
                    bool onBoundary = (x == 0 || x == MDims.x - 1 ||
                                       ...
                                       z == 0 || z == MDims.z - 1);
                    // Outside = -1, Inside = +1
                    // IsoLevel = 0 sits at the boundary
    ...
    void GenerateMesh()
    {
        ...
        // FIX 2: Only build a quad for an edge when it has a sign change.
        // Each quad connects the four cells sharing that edge.
    ...
    {
        // FIX 3: For each of the 3 axis-aligned edges at (x,y,z),
        // only emit a quad if the edge has a sign change,
        // and flip winding based on which side is solid.
